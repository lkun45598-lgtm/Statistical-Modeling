...
\subsection{研究背景}
海洋热环境变化是全球气候变暖在区域尺度上的直接体现，也是统计建模能够发挥作用的重要场景。海表温度（Sea Surface Temperature, SST）既刻画海洋上层热状态，也连接海气热量交换、季风活动、渔业资源和近海生态系统。南海处于西太平洋暖池西缘，受东亚季风、低纬海气相互作用、黑潮入侵和陆架环流共同影响，季节循环强、空间梯度大、近岸人类活动密集。因此，如何用可复现的统计指标识别南海海温增暖及其极端热风险，不仅具有气候变化研究意义，也具有海洋监测和区域治理价值。

从风险表达看，南海热环境变化并不只是年均或月均 SST 的缓慢抬升。对于珊瑚礁白化、海草床退化、近岸养殖和海洋保护区管理而言，连续多日的异常高温暴露往往比单日最高温或年平均温度更具解释力。海洋热浪（Marine Heatwave, MHW）正是针对这种持续性异常高温过程提出的统计事件定义。Hobday 等\cite{hobday2016mhw}提出的逐日分位阈值方法，使不同海域、不同季节背景下的极端热事件具有可比性，也为本文构建事件识别模型提供了统一标准。

已有研究主要从四类路径分析海洋热环境变化：一是利用卫星融合或再分析资料评估海温长期趋势；二是按统一阈值识别海洋热浪事件并比较频次、持续时间和强度；三是将 ENSO、IOD、风场和热通量等因子作为海温异常解释变量；四是面向生态系统讨论高温暴露的潜在影响\cite{oliver2018longer,holbrook2019mhw,smale2019mhw}。这些研究奠定了方法基础，但对于统计建模竞赛论文而言，仅给出单一趋势值或单一预测模型仍难以形成完整论证。优秀的统计建模工作应当同时回答四个问题：指标如何定义，模型如何建立，结论是否稳健，结果如何转化为现实解释。

基于此，本文不把研究主线设定为未来预测，而是将南海海表热环境变化抽象为两个相互联系的统计对象：一是平均态 SST/SSTA 的长期增暖，二是以持续高温事件为核心的 MHW 风险。围绕这两个对象，本文提出以下研究问题：1991--2021 年南海是否存在显著增暖；增暖是否具有空间异质性；MHW 风险是否在频次、暴露时长和累计强度上同步增强；主要结论是否对趋势方法和阈值选择稳健；气候指数和局地风场能否解释部分年际波动。

\subsection{研究思路与创新点}
本文遵循“数据基础--指标构建--模型测度--稳健检验--机制解释”的研究思路，技术路线见图 1。首先，对 OSTIA 日尺度 SST 资料进行南海区域裁剪、质量检查和月尺度聚合；其次，构造 SSTA、MHW 事件指标和相对综合热风险指数；再次，利用线性趋势、Theil-Sen 斜率、Kendall 检验和阈值敏感性分析识别平均态增暖与极端热风险变化；最后，引入气候指数和 10 m 风场资料建立解释型统计模型，对年际波动进行分析。
...
与单纯预测型或单指标描述型研究相比，本文的贡献主要体现在三个方面。第一，构建了日尺度和月尺度统一的南海海温统计分析框架，使平均态增暖测度和极端事件识别建立在同一网格基础上。第二，将 SSTA 趋势、MHW 频次、总天数、累计强度、分区比较和综合热风险指数纳入同一指标体系，形成从“是否变暖”到“风险在哪里增强”的递进论证。第三，在 OLS 趋势之外加入 Theil-Sen 斜率、Kendall 单调趋势和 85\%/90\%/95\% 阈值敏感性检验，并通过标准化回归比较风场和气候指数的解释强度，使结论具有更强的稳健性和可解释性。

\section{研究设计与数据基础}

\subsection{研究区与资料来源}
研究设计首先需要统一空间范围、时间尺度和数据口径。本文研究区设定为 $5^{\circ}$S--$25^{\circ}$N、$100^{\circ}$E--$125^{\circ}$E，覆盖南海主体海域、北部陆架、中部海盆、南部岛礁区以及低纬邻近海域。该范围能够反映南海从近岸陆架到深海盆地、从热带低纬到副热带边缘的热环境梯度。所有区域平均和分区平均均在规则经纬度网格上计算，并使用 $\cos(\varphi)$ 作为纬向面积权重。
...
资料检查发现，本地 OSTIA/Copernicus 源文件存在三个不可用月份：2014 年 7 月和 2015 年 12 月缺失全部日历日，2015 年 11 月虽有日记录但研究区海洋格点 SST 全部为缺失值。考虑到这些缺测集中于完整月份，直接插值可能人为制造或削弱连续高温事件，本文采取保守策略：月尺度趋势、相关和回归计算排除缺失月份；日尺度 MHW 识别中，缺失日和全无效日不计入有效日数，并打断连续事件。该处理方式使模型不把无观测信息误判为无风险，也为后续敏感性分析提供了明确口径。

\subsection{统计分区设定}
为避免全南海平均值掩盖空间差异，本文根据纬度、陆架海盆结构和近岸外海差异划分五个统计分区：北部陆架（$15^{\circ}$N--$25^{\circ}$N，$105^{\circ}$E--$122^{\circ}$E）、中部海盆（$8^{\circ}$N--$18^{\circ}$N，$110^{\circ}$E--$118^{\circ}$E）、南部南海（$5^{\circ}$S--$8^{\circ}$N，$105^{\circ}$E--$118^{\circ}$E）、西部近岸（$5^{\circ}$N--$22^{\circ}$N，$100^{\circ}$E--$110^{\circ}$E）和东部外海（$5^{\circ}$N--$22^{\circ}$N，$110^{\circ}$E--$125^{\circ}$E）。分区边界不代表行政区划，而是用于比较不同海洋热环境背景下统计指标的相对变化。
...
\end{equation}
其中 $m(t)$ 表示月份。SSTA 用于剔除季节循环，使长期趋势和年际波动具有可比性，是本文刻画平均态增暖的核心指标。
...
\subsection{模型检验设计}
为避免结论依赖单一模型设定，本文设置四层检验。第一，有效观测阈值和缺测排除策略用于避免缺失月份造成静默偏差。第二，OLS、Theil-Sen 和 Kendall 检验共同验证趋势方向。第三，85\%/90\%/95\% MHW 阈值比较用于检验事件定义敏感性。第四，驱动因子回归通过标准化系数、显著性和 VIF 检查解释变量的稳定性。上述设计使论文结论不是来自单次拟合结果，而是来自多种统计证据的一致性。

\section{南海海表增暖的统计测度}

\subsection{区域平均序列揭示长期增暖}
本部分首先回答南海整体是否已经出现可统计识别的长期增暖。图 2 展示 1991--2021 年南海区域平均月尺度 SST 和 SSTA，表 2 汇总主要趋势。SST 呈明显季节循环，但去除月气候态后，SSTA 仍表现出清晰上升趋势。区域平均 SST 线性趋势为 $0.191\,^{\circ}\mathrm{C}/10a$，SSTA 线性趋势为 $0.174\,^{\circ}\mathrm{C}/10a$，其中 SSTA 趋势的显著性水平远高于 0.001。这说明南海海表热状态的变化不是季节循环造成的视觉现象，而是在多年尺度上具有稳定增暖信号。
...
\subsection{趋势稳健性检验}
统计建模结论需要经受模型设定变化的检验。表 4 和图 6 对主要指标进行 OLS、Theil-Sen 和 Kendall 检验比较。区域平均 SSTA 的 OLS 趋势为 $0.174\,^{\circ}\mathrm{C}/10a$，Theil-Sen 趋势为 $0.179\,^{\circ}\mathrm{C}/10a$，Kendall $\tau=0.351$ 且 $p<0.001$。MHW 频次、总天数、累计强度和最大强度也均表现为显著增加。OLS 和 Theil-Sen 趋势虽在数值上有差异，但方向一致，说明少数异常年份并未改变长期增强结论。
...
为了将多维指标转化为可操作的空间优先级，本文构建相对综合海洋热风险指数 HRI。图 11 给出了 HRI 空间分布，表 6 汇总分区结果。北部陆架平均 HRI 为 0.574，高风险格点占比为 16.6\%，在五个统计分区中最高；西部近岸平均 HRI 为 0.537，高风险格点占比为 13.5\%。各分区的主导风险来源均为热浪暴露均值，说明长期高温暴露背景是当前综合风险排序中的关键分量。HRI 并不表示生态损失概率，但能够为监测布点和风险优先级提供统计依据。
...
为比较不同驱动因子的边际解释强度，本文进一步建立标准化回归模型。月尺度 SSTA 标准化解释模型见表 8。模型调整 $R^2$ 为 0.215，说明所选驱动因子能够解释约五分之一的月尺度 SSTA 标准化波动。Niño 3.4（滞后 3 月）和风速距平均通过显著性检验，其中风速距平系数为 $-0.363$，是稳定的负向解释变量；PDO 系数为 $-0.273$，DMI 在多元模型中未达到显著水平。
...
\subsection{主要研究结论}
本文围绕南海海表增暖和海洋热浪风险增强问题，构建了由平均态增暖测度、MHW 事件识别、分区比较、稳健性检验和驱动因子解释组成的统计建模框架。综合上述分析，得到以下结论。
...
\subsection{模型局限与展望}
本文模型仍有三个适用边界。第一，本地源数据存在 2014 年 7 月、2015 年 11 月和 2015 年 12 月三个不可用月份，虽然本文通过缺失排除、事件打断和敏感性检验降低偏差，但受影响年份的信息量仍然减少。第二，驱动因子回归使用区域平均风场和气候指数，能够识别统计联系，但不能单独证明海气过程因果链。第三，HRI 是相对统计指数，其目标是比较空间优先级，若要转化为珊瑚白化概率或渔业损失概率，还需要生态观测资料校准。后续研究可引入净热通量、混合层深度、海流、台风扰动和生态响应资料，进一步形成从物理热风险到生态损失风险的综合建模框架。
